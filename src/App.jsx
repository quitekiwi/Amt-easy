...
<style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=DM+Serif+Display:ital@0;1&display=swap'); * { box-sizing: border-box; margin: 0; padding: 0; } html, body { background: #1e2a28; } ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: #1e2a28; } ::-webkit-scrollbar-thumb { background: #9e2b1a; border-radius: 2px; } .amt-header { border-bottom: 1px solid #2e3f3c; padding: 24px 40px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; background: rgba(30,42,40,0.96); backdrop-filter: blur(12px); z-index: 100; } .amt-logo { display: flex; align-items: baseline; gap: 8px; } .amt-logo-text { font-family: 'DM Serif Display', serif; font-size: 22px; color: #d6c9b0; letter-spacing: -0.5px; } .amt-logo-dot { width: 8px; height: 8px; background: #9e2b1a; border-radius: 50%; display: inline-block; margin-left: 2px; margin-bottom: 3px; } .amt-badge { font-size: 10px; font-weight: 500; letter-spacing: 2px; text-transform: uppercase; color: #8a7a62; border: 1px solid #2e3f3c; padding: 4px 10px; border-radius: 20px; } .amt-hero { padding: 64px 40px 48px; max-width: 760px; margin: 0 auto; } .amt-eyebrow { font-size: 11px; letter-spacing: 3px; text-transform: uppercase; color: #9e2b1a; font-weight: 500; margin-bottom: 16px; } .amt-title { font-family: 'DM Serif Display', serif; font-size: clamp(32px, 5vw, 48px); line-height: 1.1; color: #d6c9b0; margin-bottom: 16px; letter-spacing: -1px; } .amt-title em { font-style: italic; color: #9e2b1a; } .amt-subtitle { font-size: 15px; color: #8a7a62; line-height: 1.6; font-weight: 300; max-width: 520px; } .amt-about { max-width: 760px; margin: 0 auto; padding: 0 40px 48px; } .amt-about-card { background: #243330; border: 1px solid #2e3f3c; border-radius: 12px; padding: 28px 32px; } .amt-about-title { font-family: 'DM Serif Display', serif; font-size: 18px; color: #d6c9b0; margin-bottom: 12px; } .amt-about-text { font-size: 13px; color: #8a7a62; line-height: 1.8; font-weight: 300; margin-bottom: 16px; } .amt-about-legal { font-size: 11px; color: #4a5f5c; line-height: 1.7; padding-top: 16px; border-top: 1px solid #2e3f3c; } .amt-main { max-width: 760px; margin: 0 auto; padding: 0 40px 80px; } .amt-card { background: #243330; border: 1px solid #2e3f3c; border-radius: 12px; padding: 28px; margin-bottom: 16px; } .amt-card-label { font-size: 10px; letter-spacing: 2.5px; text-transform: uppercase; color: #4a5f5c; font-weight: 500; margin-bottom: 14px; } .amt-textarea { width: 100%; background: #1e2a28; border: 1px solid #2e3f3c; border-radius: 8px; color: #d6c9b0; font-family: 'DM Sans', sans-serif; font-size: 14px; line-height: 1.7; padding: 16px; resize: vertical; min-height: 160px; outline: none; transition: border-color 0.2s; font-weight: 300; } .amt-textarea:focus { border-color: #9e2b1a; } .amt-textarea::placeholder { color: #3a4f4c; } .amt-char-count { font-size: 11px; text-align: right; margin-top: 6px; transition: color 0.2s; } .amt-controls { display: flex; gap: 12px; align-items: center; margin-top: 16px; flex-wrap: wrap; } .amt-lang-select { position: relative; } .amt-lang-btn { background: #1e2a28; border: 1px solid #2e3f3c; border-radius: 8px; color: #d6c9b0; font-family: 'DM Sans', sans-serif; font-size: 13px; padding: 10px 16px; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: border-color 0.2s; white-space: nowrap; } .amt-lang-btn:hover { border-color: #4a6f6a; } .amt-lang-dropdown { position: absolute; bottom: calc(100% + 6px); top: auto; left: 0; background: #243330; border: 1px solid #2e3f3c; border-radius: 8px; overflow-y: auto; max-height: 280px; z-index: 200; min-width: 160px; box-shadow: 0 8px 32px rgba(0,0,0,0.5); } .amt-lang-option { padding: 10px 16px; cursor: pointer; font-size: 13px; display: flex; align-items: center; gap: 10px; transition: background 0.15s; } .amt-lang-option:hover { background: #2e3f3c; } .amt-lang-option.active { color: #9e2b1a; } .amt-analyze-btn { background: #9e2b1a; color: #d6c9b0; border: none; border-radius: 8px; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; letter-spacing: 0.5px; padding: 11px 28px; cursor: pointer; transition: all 0.2s; flex: 1; } .amt-analyze-btn:hover:not(:disabled) { background: #7d2015; transform: translateY(-1px); } .amt-analyze-btn:disabled { opacity: 0.35; cursor: not-allowed; } .amt-examples { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 12px; } .amt-example-btn { background: transparent; border: 1px solid #2e3f3c; border-radius: 20px; color: #4a5f5c; font-family: 'DM Sans', sans-serif; font-size: 11px; padding: 5px 12px; cursor: pointer; transition: all 0.2s; letter-spacing: 0.3px; } .amt-example-btn:hover { border-color: #4a6f6a; color: #8a9a97; } .amt-loading { display: flex; align-items: center; gap: 12px; padding: 32px; justify-content: center; color: #4a5f5c; font-size: 13px; letter-spacing: 1px; } .amt-spinner { width: 18px; height: 18px; border: 2px solid #2e3f3c; border-top-color: #9e2b1a; border-radius: 50%; animation: spin 0.8s linear infinite; } @keyframes spin { to { transform: rotate(360deg); } } .amt-result { animation: fadeUp 0.4s ease; } @keyframes fadeUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } } .amt-result-header { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 24px; gap: 16px; flex-wrap: wrap; } .amt-doc-type { font-family: 'DM Serif Display', serif; font-size: 20px; color: #d6c9b0; } .amt-urgency-pill { font-size: 11px; font-weight: 600; letter-spacing: 1px; padding: 5px 12px; border-radius: 20px; white-space: nowrap; } .amt-disclaimer-top { font-size: 11px; color: #4a5f5c; line-height: 1.6; padding: 10px 14px; border: 1px solid #2e3f3c; border-radius: 6px; margin-bottom: 24px; } .amt-summary { font-size: 14px; line-height: 1.75; color: #8a9a97; font-weight: 300; padding: 16px; background: #1e2a28; border-left: 2px solid #9e2b1a; border-radius: 0 6px 6px 0; margin-bottom: 24px; } .amt-section { margin-bottom: 24px; } .amt-section-title { font-size: 10px; letter-spacing: 2.5px; text-transform: uppercase; color: #4a5f5c; font-weight: 500; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid #2e3f3c; } .amt-deadline-item { display: flex; gap: 14px; align-items: flex-start; padding: 10px 0; border-bottom: 1px solid #243330; } .amt-deadline-date { font-size: 12px; font-weight: 600; color: #9e2b1a; white-space: nowrap; min-width: 90px; padding-top: 1px; } .amt-deadline-desc { font-size: 13px; color: #8a9a97; line-height: 1.5; font-weight: 300; } .amt-checklist-item { display: flex; align-items: flex-start; gap: 12px; padding: 8px 0; font-size: 13px; color: #8a9a97; line-height: 1.5; font-weight: 300; } .amt-check-box { width: 16px; height: 16px; border: 1.5px solid #3a5550; border-radius: 4px; flex-shrink: 0; margin-top: 2px; cursor: pointer; transition: all 0.15s; } .amt-check-box:hover { border-color: #9e2b1a; } .amt-check-box.checked { background: #9e2b1a; border-color: #9e2b1a; display: flex; align-items: center; justify-content: center; } .amt-office-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; } .amt-office-item { background: #1e2a28; border-radius: 6px; padding: 12px 14px; } .amt-office-label { font-size: 10px; color: #4a5f5c; letter-spacing: 1.5px; text-transform: uppercase; margin-bottom: 4px; } .amt-office-value { font-size: 13px; color: #b8ab95; font-weight: 400; } .amt-number-item { display: flex; justify-content: space-between; align-items: center; padding: 10px 14px; background: #1e2a28; border-radius: 6px; margin-bottom: 6px; } .amt-number-label { font-size: 12px; color: #4a5f5c; font-weight: 400; } .amt-number-value { font-size: 13px; color: #d6c9b0; font-weight: 600; font-variant-numeric: tabular-nums; letter-spacing: 0.5px; } .amt-error { color: #9e2b1a; font-size: 13px; padding: 16px; text-align: center; } @media (max-width: 600px) { .amt-header { padding: 18px 20px; } .amt-hero { padding: 40px 20px 32px; } .amt-about { padding: 0 20px 32px; } .amt-main { padding: 0 20px 60px; } .amt-office-grid { grid-template-columns: 1fr; } .amt-controls { flex-direction: column; } .amt-analyze-btn { width: 100%; } }`}</style>
