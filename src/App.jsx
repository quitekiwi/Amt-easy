    ...
    <div style={{ minHeight: "100vh", background: "#1e2a28", fontFamily: "'DM Sans', sans-serif", color: "#d6c9b0", padding: "0" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 600px) { .amt-header { padding: 18px 20px; } .amt-hero { padding: 40px 20px 32px; } .amt-main { padding: 0 20px 60px; } .amt-office-grid { grid-template-columns: 1fr; } .amt-controls { flex-direction: column; } .amt-analyze-btn { width: 100%; } }
      `}</style>
          ...
          <div className="amt-controls">
            <div className="amt-lang-select">
              <button className="amt-lang-btn" onClick={() => setLangOpen(!langOpen)}>
                ...
                <span style={{ color: "#4a5f5c", fontSize: 10 }}>v</span>
              </button>
              {langOpen && (
  ...
  return (
    <div className="amt-checklist-item">
      <div className={"amt-check-box" + (checked ? " checked" : "")} onClick={() => setChecked(!checked)}>
        {checked && <span style={{ color: "#d6c9b0", fontSize: 10, fontWeight: 700 }}>v</span>}
      </div>
